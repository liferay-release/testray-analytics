...
# Release-engineering bookkeeping: regenerated by bots on nearly every commit
# and never a root cause. These stay excluded unconditionally.
GIT_DIFF_EXCLUDES = [
    ":!**/artifact.properties",
    ":!**/.releng/**",
...
# Previously also excluded: *.scss, *.css, *.gradle, *.xml, *.properties,
# *.json, *.yml, *.yaml, *.tf, *.sh, bnd.bnd.
#
# That list was tuned for *test* failures, where config churn is noise. Stable's
# failure mode is *build breakage*, where these types ARE the root cause, so the
# filter removed exactly the evidence Stable needs. Build 509462694 is the proof:
# the culprit was a deleted
#   .../clay-css/src/scss/_atlas-custom-properties-variables.scss
# (commit d4241309c8c10, a revert of LPD-92565) that object-web still @imports,
# breaking dart-sass. `:!**/*.scss` meant the diff could not contain it, so no
# amount of rubric tuning could have found it. In that range the old list dropped
# 154 of 298 changed files: scss 32, gradle 24, xml 16, properties 10, json 2.
#
# Cost impact is near zero: the prompt is built from `hunks.txt` (relevance
...
- **POSSIBLE_BUG** — probably a genuine defect, diff-caused, at **`medium`** confidence you cannot verify to `high`. **The claim is about the FAILURE, not about how precisely you can attribute it**: a server 500 with three candidate causes is still probably a bug. **Name the single candidate in `culprit_file` when there is exactly one; with two or more, leave `culprit_file` null and list them all in `specific_change`, `; `-separated.** Only when you cannot point at any candidate at all — or cannot tell a defect from an intentional change — does it drop to NEEDS_REVIEW.
- **The misbehaviour trigger \u2014 read this before reaching for NEEDS_REVIEW.** If the error shows the PRODUCT doing something wrong \u2014 an HTTP 500, an element that should exist and does not, a wrong or missing value, a stack trace from product code \u2014 **and** the area it touches was changed in this range, that is **POSSIBLE_BUG**, not NEEDS_REVIEW and not FALSE_POSITIVE. Two mistakes this exists to stop, both observed on real runs: calling a 500 \u201cenvironmental\u201d, and calling a CKEditor timeout a \u201ctiming flake\u201d in a range that rewrote CKEditor. NEEDS_REVIEW is for when you cannot tell whether anything is broken at all \u2014 not for when something is clearly broken and you cannot say which change did it. **Adjacency is not causation.** It is NOT enough that *something* in the area changed: over a wide range almost everything changed, so \u201cthe module was touched\u201d proves nothing. The change must plausibly cause THIS symptom. A transitive or adjacent change \u2014 the module is nearby, the test imports something in it \u2014 is a NEEDS_REVIEW candidate, not a misbehaviour trigger.
- **The enforcement trigger \u2014 also read this before reaching for NEEDS_REVIEW.** A build-enforcement check that FAILS and names what it wants \u2014 semantic versioning (`VERSION INCREASE REQUIRED`, a package with CUR_VER/BASE_VER/REC_VER), a baseline report, source formatting, a compile error naming the symbol \u2014 is a **BUG** at `high` confidence. Its own output is the evidence, and the fix it states (bump `com.liferay.portal.kernel.util` to 102.1.0, regenerate the baseline) is actionable without knowing who caused it. **Attribution is a separate question from classification here:** when no commit in range added the offending member, leave `culprit_file` and `culprit_commit` null, say in `specific_change` that the API change predates the range, and still classify BUG. Observed twice on build 19999 of Stable: the model wrote \u201cit is a genuine defect (missing version/packageinfo update)\u201d and then filed NEEDS_REVIEW because it could not name the commit \u2014 which reads to a human as \u201cwe do not know if anything is wrong\u201d, when the tool had already said exactly what is wrong and how to fix it.
...
- **NEEDS_REVIEW** — the safe default when you can't narrow to a single culprit. Any of:
  - **Cannot tell a defect from an intentional change** — you have candidates but no way to judge whether production is wrong or merely ahead of its tests. List ALL candidates separated by `; ` in `specific_change`. Note that *many candidates* alone does NOT put a failure here: if it is probably a defect, that is POSSIBLE_BUG with the candidates listed.
...
- **FALSE_POSITIVE** — clearly environmental or genuinely unrelated. May be `high` confidence (timeouts, gradle build infrastructure, chrome version, TEST_SETUP_ERROR are confidently environmental). Common patterns:
  **FALSE_POSITIVE stays reachable however much changed.** Two fingerprints outrank “something in the area changed”, and both were observed dismissing real flakes correctly:
  - **One error signature spanning unrelated components or teams** — a single product change does not break kaleo-designer, marketplace and analytics identically, nor change-tracking, DDM forms and publications. That shape is infrastructure.
  - **An infrastructure symptom with no change that plausibly causes it** — `net::ERR_ABORTED`, “browser has been closed”, “Login via API failed”, a bare `java.util.concurrent.ExecutionException`, or a seed/mock backend returning 500.
  Where either holds, FALSE_POSITIVE is right even though relevant-looking code changed.
...
- **NEEDS_REVIEW** — the safe default when you can't narrow to a single culprit. Any of:
  - **Cannot tell a defect from an intentional change** — candidates exist but you cannot judge whether production is wrong. List ALL candidates separated by `; ` in `specific_change`. Many candidates alone does NOT put a failure here: probably a defect means POSSIBLE_BUG with the candidates listed.
